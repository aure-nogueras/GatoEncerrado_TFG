...
module.exports = {
    ...
    interaction(object, conv){
        ...
        switch(object){
            case 'estantería':
                speechText = 'Exploras la estantería y  encuentras un juego de mesa. Por lo demás, hay unos cuantos libros polvorientos, pero no son de tu interés. Dime qué quieres hacer ahora.  '
                break;
            case 'juego de mesa':
                speechText = 'Se trata de una extraña versión del Cluedo. Se ha cometido el asesinato de un gato. Alguien ha dejado una partida a medias. '
                    + 'Miras la solución. El gato ha sido asesinado por un perro, con atún envenenado en el arenero. Vaya, curiosa versión. Dime qué quieres hacer ahora.  '
                break;
            case 'arenero':
                if(floorObjectsBasement.indexOf('llaves coche') === -1 && objectsBasement.indexOf('llaves coche') !== -1){
                    speechText = 'No te resulta especialmente agradable, pero metes la mano en el arenero. Encuentras algo y deseas que no sea lo que estás pensando. Por suerte, sacas la mano y encuentras unas llaves de coche. Dime qué quieres hacer ahora.  '
                    ...
                    speechText = 'No hay nada en el arenero. Dime qué quieres hacer ahora.  '
                }
                break;
            case 'coche':
                if(basementElements.indexOf('coche') === -1){
                    speechText = 'Se trata de un coche rojo. Está cerrado y no puedes ver nada de su interior. Dime qué quieres hacer ahora.  '
                }else{
                ...
                break;
            case 'interruptor':
                speechText = 'Pulsas el interruptor, pero no funciona. Dime qué quieres hacer ahora.  '
                break;
            case 'escaleras':
                speechText = 'Las escaleras suben hasta el salón. Dime qué quieres hacer ahora.  ';    
                break;
            case 'lámpara':
                speechText = 'La lámpara está encendida. No hay nada especial en ella. Dime qué quieres hacer ahora.  ';    
                break;
            case 'puerta':
    ...
    use(object, element, conv){
        ...
        if(object === 'llaves coche' && element === 'coche'){
            if(basementElements.indexOf('coche') === -1){
                speakOutput = '<speak> Abres el coche.' 
                    + '<audio src="https://actions.google.com/sounds/v1/transportation/opening_closing_car_door_far.ogg"/>'
                basementElements.push('coche');
                conv.basementElements = basementElements;
                points += 10;
            }else{
                speakOutput = '<speak> El coche está abierto. ';
            }
            if(floorObjectsBasement.indexOf('gato') === -1 && objectsBasement.indexOf('gato') !== -1){
                speakOutput += 'Miras dentro. En el maletero hay un gato muy mono. Te enamoras de él nada más verlo. ';
                if(basementElements.indexOf('gato') === -1){
                    speakOutput += 'Te mira esperando algo. Para darle objetos, di dar y los objetos que quieras darle. Puedes darle entre uno y cuatro objetos al gato. '
                    + ' Si le das cuatro objetos, debes seguir un orden específico que has ido descubriendo a lo largo de tu aventura. </speak>'
                }else{
                    speakOutput += ' Dime qué quieres hacer ahora. </speak>';
                }
            }else{
                speakOutput += 'Ya no hay nada dentro. Dime qué quieres hacer ahora. </speak>'
            }
        }
        if(object === 'mando garaje' && element === 'puerta'){
            if(basementElements.indexOf('puerta') === -1){
                speakOutput = 
                    '<speak> Usas el mando del garaje y abres la puerta.' 
    ...
    giveCats(element1, element2, element3, element4, conv){
        let speakOutput = 'El coche está cerrado. Dime qué quieres hacer ahora. ';
        points = 0;
        let success = false;
        if(basementElements.indexOf('coche') !== -1){
            if(basementElements.indexOf('gato') === -1){
                if(harmed){
                    speakOutput = 'No puedes probar otra vez. Estás herido... Dime qué quieres hacer ahora. ';
                }else{
                    if(element1 === 'cuenco' && element2 === 'comida de gato' && element3 === 'ovillo de lana' && element4 === 'ratón de juguete'){
                        speakOutput = "<speak>" 
                            ...
                            + ' El gato deja caer al suelo el mando del garaje. Dime qué quieres hacer ahora. '
                            + "</speak>";
                        ...
                        if(!harmed){
                            speakOutput = "<speak> " + element1 + ' ' + element2 + ' ' + element3 + ' ' + element4 + ' '
                                ...
                                + 'Debes detener la sangre de alguna forma o te desmayarás. Dime qué quieres hacer ahora. </speak>';
               ...
               speakOutput = 'Ya le has dado los objetos al gato de forma correcta. Está feliz. Dime qué quieres hacer ahora. '; 
            }
        }
        return {
            speakOutput: speakOutput,
            ...
            if(basementElements.indexOf('gato') === -1){
                if(harmed){
                    ...
                    if(element1 !== element2 && element2 !== element3 && element1 !== element3){
                        ...
                        speakOutput = 'Los objetos no pueden repetirse. Dime qué quieres hacer ahora. ';
                    }
                }
            }else{
               speakOutput = 'Ya le has dado los objetos al gato de forma correcta. Está feliz. Dime qué quieres hacer ahora. '; 
        ...
        if(basementElements.indexOf('coche') !== -1){
            if(basementElements.indexOf('gato') === -1){
                if(harmed){
                    ...
                    speakOutput = '<speak> Solo le has dado un objeto de cuatro al gato. '
                    + '<audio src="https://actions.google.com/sounds/v1/horror/monster_alien_grunt_hiss.ogg"/>'
    ...
    clue(conv){
        ...
        if(n === 0){
            ...
            speakOutput = 'La clave del orden está en unos mensajes misteriosos que has ido recibiendo a lo largo del recorrido. ';
            n = 4;
        }else if(n === 4){
